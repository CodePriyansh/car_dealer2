    ...
    return (
      <div className={styles.wrapper}>
        ...
        <div className={styles.right_wrapper}>
          {/* Top headings */}
          <button className="self-start mb-8 bg-none border-none text-blue-500 cursor-pointer" onClick={() => router.back()}>Back</button>
          <Image
            src={Images.myCar}
            alt="logo"
            width={32}
          ...
          <p className={styles.heading}>{isOtpEntered ? "Confirm Otp": "Otp Verification" }</p>
          <form onSubmit={handleSubmit} className="flex flex-col items-center">
            ...
            {!isOtpEntered && (
              <div className="flex gap-4 items-center mb-4 flex-col text-base">
                <span className="text-lg text-primary">00:{seconds < 10 ? `0${seconds}` : seconds}</span>
                <button type="button" className="bg-none border-none  text-greyy cursor-pointer font-semibold" onClick={handleResend}>Resend OTP</button>
              </div>
            )}
            {isOtpEntered && (
              <button type="submit">
