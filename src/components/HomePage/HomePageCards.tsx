  ...
  return (
    <>
      {cars?.length < 1  ? (
        ...
        <>
        <p className="text-para font-bold block mt-2 sm:text-[24px] font-rajdhani container_space ">{cars?.length} {activeFilter}s</p>
        <div className="container_space  w-full grid md1:grid-cols-3 sm:grid-cols-2 grid-cols-1  sm:gap-6 gap-4 md:bg-f7f7f7 sm:pb-4 py-2">
          ...
          {cars?.map((car:any, index:number) => (
            <CarCards item={car} key={index} onDelete={onDelete}  />
          ))}
        </div>
        </>
